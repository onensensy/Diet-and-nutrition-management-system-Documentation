...
In this phase, all requirements of the project are analyzed and documented in a specification document and a feasibility analysis is done to check if these requirements are valid. At this stage, as developers we shall describe and analyze the needs and behavior of the user towards the product. We will conduct research on potential users as to what they want when opening and entering the website. Potential users are referred to as subjects who want to diet, do a healthy life.

Also, at this stage will be analyzed how this system will meet user needs when using this website to place an order. The requirements gathered will be both functional and non-functional requirements. For example Functional include “The system should authenticate a user’s new account on in order to gain access.” Non-functional “The system should be able to handle 50 users without performance deterioration.” and others. After thorough analysis, a Requirements Understanding Documents (RUD) is created.

We will make use of tools such as frameworks for both front end and back end to develop the system along with languages such as JavaScript, CSS and HTML for frontend and PHP, JavaScript for the backend logic ,SQL for the database operation  and servers like the MySQL server and MySQL database for management.
...
\subsubsection{Use Case Diagram Web Application.}
...
The UML Class Diagram on this website consists of seven classes as shown i below. Each class has different responsibilities and functions commonly referred to as attributes and operations. User class is used as login or login to the website, divided into admin and customer. Admin and customer classes are the users' bio data. Class Order to store data about an order. Payment class for storing and sending proof of payment. Product class is used to store biodata of available goods and stock availability. Shopping Cart class is used to store food datain the long run.
